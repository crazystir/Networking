...
int main() {
    ...
    if (!fork()) {
        ...
        DNSServer udpServer(UDP, private_ip, DEFAULT_SERVER_PORT, 10, 8080, public_ip, (char*)UPSTREAM_ADDR, UPSTREAM_PORT);
        udpServer.start();
        exit(0);
    }
    if (!fork()) {
        printf("The TCP DNS server\n");
        DNSServer tcpServer(TCP, private_ip, DEFAULT_SERVER_PORT, 10, 8081, public_ip, (char*)UPSTREAM_ADDR, UPSTREAM_PORT);
        tcpServer.start();
        exit(0);
    }
    if (!fork()) {
        printf("The HTTP server\n");
        HTTPServer httpServer(TCP, private_ip, 80, DEFAULT_SERVER_BACKLOG, public_ip, nullptr, 0);
        httpServer.start();
        exit(0);
    }
    if (wait(nullptr) < 0) {
