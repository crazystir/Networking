...
int main() {
    ...
    if (!fork()) {
        ...
        DNSServer udpServer(UDP, private_ip, DEFAULT_SERVER_PORT, 10, 5000, public_ip, (char*)UPSTREAM_ADDR, UPSTREAM_PORT);
        udpServer.start();
        exit(0);
    }
    if (!fork()) {
        printf("The TCP DNS server\n");
        DNSServer tcpServer(TCP, private_ip, DEFAULT_SERVER_PORT, 10, 7000, public_ip, (char*)UPSTREAM_ADDR, UPSTREAM_PORT);
        tcpServer.start();
        exit(0);
    }
    if (!fork()) {
